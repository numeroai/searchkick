  ...
  module Model
    def searchkick(**options)
      ...
      unknown_keywords = options.keys - [:_all, :_type, :batch_size, :callbacks, :callback_options, :case_sensitive, :conversions, :conversions_v2, :deep_paging, :default_fields,
        :client_name, :filterable, :geo_shape, :highlight, :ignore_above, :index_name, :index_prefix, :inheritance, :job_options, :knn, :language,
      ...
      include(mod)
      mod.module_eval do
        def reindex(method_name = nil, mode: nil, refresh: false, ignore_missing: nil, on_missing: nil, full_reindex_method_name: nil, job_options: nil)
          self.class.searchkick_index.reindex([self], method_name: method_name, mode: mode, refresh: refresh, on_missing: on_missing, ignore_missing: ignore_missing, full_reindex_method_name: full_reindex_method_name, job_options: job_options, single: true)
